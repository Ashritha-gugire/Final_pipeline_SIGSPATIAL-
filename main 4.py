...
def stage_models(
    ...
    Args:
        X_normal:      StandardScaler-transformed feature matrix of normal agents
                       only. Shape (n_normal, n_features). This is the clean
                       training set — unsupervised models learn the baseline
                       distribution from these agents exclusively.
        X_all:         StandardScaler-transformed feature matrix for ALL agents
                       (normal + anomalous). Shape (n_total, n_features). Every
                       fitted model produces one anomaly score per row of this
                       matrix at inference time.
        labels:        Binary ground-truth array aligned with X_all rows.
                       1 = anomalous, 0 = normal. Used post-hoc to orient
                       scores (ensure higher = more anomalous) and to train the
                       supervised XGBoost† reference model.
        checkpoints:   Directory path where fitted model artifacts are
                       serialised (pickle/pt files) for reproducibility and
                       --skip-training reload.
        args:          CLI argparse namespace. Checked for skip_training and
                       no_deep flags to control which models run.
        log:           Logger instance for progress/timing messages.
        dataset_label: Human-readable prefix for log lines (e.g. "YJMob-5%").
    """
...
def run_yjmob(args, log: logging.Logger) -> None:
    ...
    # ── Subsample agents to fit in memory (keep only what's needed) ───────────
    max_norm = MAX_AGENTS_YJMOB - int(MAX_AGENTS_YJMOB * min(CONTAMINATION_RATES))
    max_anom = int(MAX_AGENTS_YJMOB * max(CONTAMINATION_RATES))
    nonanom_ids = pd.Series(past_nonanom["agent"].unique())
    anom_ids    = pd.Series(past_anom["agent"].unique())
    if len(nonanom_ids) > max_norm:
        nonanom_ids = nonanom_ids.sample(n=max_norm, random_state=RANDOM_SEED)
    if len(anom_ids) > max_anom:
        anom_ids = anom_ids.sample(n=max_anom, random_state=RANDOM_SEED)
    _keep_nonanom  = set(nonanom_ids.values)
    _keep_anom     = set(anom_ids.values)
    past_nonanom   = past_nonanom[past_nonanom["agent"].isin(_keep_nonanom)]
    future_nonanom = future_nonanom[future_nonanom["agent"].isin(_keep_nonanom)]
    past_anom      = past_anom[past_anom["agent"].isin(_keep_anom)]
    future_anom    = future_anom[future_anom["agent"].isin(_keep_anom)]
    log.info(f"[YJMob] Subsampled: {len(_keep_nonanom):,} normal + "
             f"{len(_keep_anom):,} anomalous agents")
    ...
    for rate, rate_label in zip(CONTAMINATION_RATES, CONTAMINATION_LABELS):
        ...
        if args.sequence_models:
            ...
            # df_norm_s["agent"] has original IDs; df_anom_s["agent"] has offset IDs.
            # Stays DataFrames use original IDs, so filter with un-offset anom IDs.
            orig_norm_ids = set(df_norm_s["agent"])
            orig_anom_ids = {a - max_id for a in df_anom_s["agent"]}

            past_norm_s   = past_nonanom[past_nonanom["agent"].isin(orig_norm_ids)]
            past_anom_s   = past_anom[past_anom["agent"].isin(orig_anom_ids)].copy()
            past_anom_s["agent"] = past_anom_s["agent"] + max_id

            future_norm_s = future_nonanom[future_nonanom["agent"].isin(orig_norm_ids)]
            future_anom_s = future_anom[future_anom["agent"].isin(orig_anom_ids)].copy()
            future_anom_s["agent"] = future_anom_s["agent"] + max_id

            past_sample   = pd.concat([past_norm_s, past_anom_s], ignore_index=True)
            future_sample = pd.concat([future_norm_s, future_anom_s], ignore_index=True)
